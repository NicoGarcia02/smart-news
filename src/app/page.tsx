...
export default function Home() {
  ...
  const handleFeedback = useCallback(
    async (item: NewsItem, action: "like" | "dislike") => {
  ...
  function renderMarkdown(text: string) {
    return text
      ...
      .replace(/^- (.+)$/gm, '<li style="color:#8a96b0;font-size:0.8125rem;margin:0.25rem 0;padding-left:0.25rem;">$1</li>')
        ...
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div>
              ...
              <p className="text-[11px] mt-0.5 capitalize" style={{ color: "var(--muted)" }}>
                {formatDate()}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <span
                className="text-sm"
                style={{ display: "inline-block", animation: loadingNews ? "spin 1s linear infinite" : "none" }}
              >
                ⟳
              </span>
      ...
      >
        <div className="max-w-5xl mx-auto flex">
          {(["summary", "news"] as Tab[]).map((t) => (
            ...
            >
              {t === "summary" ? "Resumen del Día" : `Noticias${news.length ? ` · ${news.length}` : ""}`}
              {tab === t && (
                <span
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-5 sm:py-8 pb-24 sm:pb-10">
        ...
        {tab === "summary" && (
          <div className="relative">
            <div
              ref={summaryRef}
              className="rounded-2xl p-7 min-h-80 max-h-[72vh] overflow-y-auto"
              style={{
                background: "var(--surface)",
                border: "1px solid var(--border)",
              }}
                ...
                <div className="flex flex-col items-center justify-center h-60 gap-4">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-xs mt-1" style={{ color: "var(--muted)" }}>
                      {news.length > 0
                        ? `${news.length} noticias listas para analizar`
                        : "Primero actualizá las noticias"}
                    </p>
                  </div>
                </div>
              )}
            ...
            {news.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-72 gap-4">
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs mt-1" style={{ color: "var(--muted)" }}>
                    Presioná "Actualizar" en la barra superior
                  </p>
                </div>
              </div>
            ) : (
